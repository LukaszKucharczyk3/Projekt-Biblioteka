...
void pokazUzytkownikow() {
    ...
    while(plik.read((char*)&u, sizeof(Uzytkownik))) {
        ...
        cout << "Imie i nazwisko: " << u.imieNazwisko << "\n";
        cout << "Adres: " << u.adres << "\n";
        cout << "------------------------------\n";
    }
    plik.close();
...
void wypozyczKsiazke() {
    ...
    if(!plikKs) {
        cout << RED << "Brak ksiazki.bin." << RESET << endl;
        return;
    }
    Ksiazka ks;
    bool znaleziono = false;
...
void oddajKsiazke() {
    ...
    Wypozyczenie w;
    while(in.read(reinterpret_cast<char*>(&w), sizeof(Wypozyczenie))) {
        if(strcmp(w.numerID, numerID) == 0 && strcmp(w.tytul, tytul) == 0 && w.data_zwrotu == 0) {
            char stan;
        ...
        out.write(reinterpret_cast<char*>(&w), sizeof(Wypozyczenie));
    }
    in.close();
    out.close();
    if(!znaleziono) {
    ...
    while(true) {
        pos = plikKs.tellg();
        if(!plikKs.read(reinterpret_cast<char*>(&k), sizeof(Ksiazka))) break;
        if(strcmp(k.tytul, tytul) == 0) {
            k.ilosc += 1;
            plikKs.seekp(pos);
            plikKs.write(reinterpret_cast<char*>(&k), sizeof(Ksiazka));
            break;
        }
    }
    plikKs.close();
            ...
            if(!in) {
                ...
                while(in.read((char*)&w, sizeof(Wypozyczenie))) {
                    if(strcmp(w.numerID, id) == 0 && w.data_zwrotu == 0) {
                        cout << "Tytul: " << w.tytul
                             << ", wypozyczono: " << ctime(&w.data_wypozyczenia);
                        znaleziono = true;
                    }
                }
                in.close();
    ...
    while (plikKs.read((char*)&ks, sizeof(Ksiazka))) {
        if (ks.ilosc < ks.prog_zamowienia) {
            ...
            while (dostawy.read((char*)&d, sizeof(Dostawa))) {
                if (strcmp(d.tytul, newDostawa.tytul) == 0 && strcmp(d.autor, newDostawa.autor) == 0) {
...
void reczneZamawianie() {
    ...
    while (dostawy.read((char*)&existing, sizeof(Dostawa))) {
        if (strcmp(existing.tytul, d.tytul) == 0 && strcmp(existing.autor, d.autor) == 0) {
...
void zatwierdzDostawe() {
    ...
    if(!in) {
        cout << RED << "Brak w_dostawie.bin." << RESET << endl;
        return;
    }
    ...
    do {
        ...
        switch(wybor) {
            case 1:
            {
                ...
                while(in.read((char*)&d, sizeof(Dostawa))) {
                    cos = true;
                    cout << GREEN << "Tytul: " << RESET << d.tytul
                         << ", Autor: " << d.autor
                         << ", Ilosc: " << d.ilosc
